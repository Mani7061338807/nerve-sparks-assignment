<script>
  ...
  import { navigate } from 'svelte-routing';
    ...
    const handleSubmit = async () => {
     try {
        const response = await axios.post('http://localhost:3000/dealership/signup', {
  ...
  <div class="w-[100vw] min-h-[100vh] bg-[#040517] flex justify-center items-center">
    ...
    <form class="flex w-full md:w-[80%] relative m-auto justify-center items-center flex-col" on:submit|preventDefault={handleSubmit}>
     ...
     <div class="w-[90%] mb-8 h-[50px]">
         <label class="w-full font-medium text-gray-500 dark:text-gray-400 [&>*]:mt-[0.35rem]" for="Location">Location:</label>
       <input class="w-full mt-1 z-[1] bg-[#202140] rounded-md text-[white] h-[40px] border-[1px] border-[gray] pl-3" type="text" id="Location" bind:value={Location} required />
     </div>
     <div class="w-[90%] mb-8 h-[50px]">
         <label class="w-full font-medium text-gray-500 dark:text-gray-400 [&>*]:mt-[0.35rem]" for="DealerShipId">DealerShip Id:</label>
       <input class="w-full mt-1 z-[1] bg-[#202140] rounded-md text-[white] h-[40px] border-[1px] border-[gray] pl-3" type="text" id="DealerShipId" bind:value={DealerShipId} required />
     </div>
